...
function GenerateRating($rating, $location) {
	...
	if ($location == 'view_game') {
		$empty_star = '<img src="'.$setting['site_url'].$setting['template_url'].'/images/'.$template['view_game_empty_star'].'" alt="Rating star" />';
		$half_star = '<img src="'.$setting['site_url'].$setting['template_url'].'/images/'.$template['view_game_half_star'].'" alt="Rating star" />';
		$star = '<img src="'.$setting['site_url'].$setting['template_url'].'/images/'.$template['view_game_star'].'" alt="Rating star" />';
	}
...
function GameData($raw_data, $type) {
	...
	$game = array('id' => $raw_data['id'], 'instructions' => nl2br($raw_data['instructions']), 'plays' => $raw_data['hits'],  'highscores' => $raw_data['highscores'],  'seo_url' => $raw_data['seo_url'],  'category' => $raw_data['category_id'], 'submitter' => $raw_data['submitter'], 'featured' => $raw_data['featured']);
