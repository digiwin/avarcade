...
if ($lb_count != 0) {
	...
	if ($count) {
		...
		if ($total_pages != 1) {
			...
			if ($page != 1) {
				$prev_page = $page - 1;
				echo '<a href="#" onclick="HighscorePage('.$id.', '.$prev_page.', \''.$lb_id.'\', \''.$setting['site_url'].'\', 2); return false">< '.PREVIOUS.'</a> &nbsp;';
			}
			if ($page != $total_pages) {
				$next_page = $page + 1;
				echo ' <a href="#" onclick="HighscorePage('.$id.', '.$next_page.', \''.$lb_id.'\', \''.$setting['site_url'].'\', 2); return false">'.NEXT.' ></a>';
			}
			echo '</div>';
		}
	} else {
